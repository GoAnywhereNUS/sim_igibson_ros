...
from igibson.utils.assets_utils import get_3dfront_scene_path, get_cubicasa_scene_path, get_ig_scene_path, get_scene_path
...
def generate_trav_map(scene_name, scene_source, load_full_scene=True):
    if scene_source == "G":
        scene_dir = get_scene_path(scene_name)
        scene = StaticIndoorScene(scene_name, build_graph=False)
    else:
        if scene_source == "IG":
    ...
    if load_full_scene and scene_source != "G":
